...
from config_cal import email_from, base_path, web_path, \
     act_db, HD_db, months_ini
            ...
            if len(act_values) >= 5:
                ...
                if cal_day >= cur_day:
                    ...
                    if 'control' in cal_path:
                        ...
                        # public pages, remove link
                        if show_loc == 'On':
                            content_list.append('{0}<br/>{1}</a><br/>\n'.format(activity, location))
                        else:
                            content_list.append('{0}</a><br/>\n'.format(activity))
                else:
                    # events are in the past, abbreviate and use italic
                    content_list.append('<i>{0}<br/>{1} <br/></i>\n'.format(time_startfmt, activity))

    if 'control' in cal_path:
        # show Add Activity at bottom of cell for all dates in month
        if cal_day >= month_object:

            if len(content_list) >= 3:
                content_list.append('''<br/><a class="page" href="/shc/akc/edit_act.cgi?start_date={0}">
                Add Activity</a>\n'''.format(cal_day))
            else:
                #Add empty lines if the day is empty
                content_list.append('''<br/><br/><a class="page" href="/shc/akc/edit_act.cgi?start_date={0}">
                Add Activity</a>\n'''.format(cal_day))
        ...
        for x in xrange(0,7):
            ...
            if day == 0:
                ...
                if content_list and len(content_list) > 0:
                    ...
                    #no events
                    target.write('<td align="center"><table width="140"><tr><td class="num">{0}</td></tr>\
                                 \n<tr><td class="weekday"></td></tr></table></td>\n'.format(day))

    target.write('</table><br/></body></html>')
    target.close()

    if dtg.year == cur_day.year and dtg.month == cur_day.month:
        current_control = os.path.join(base_path, 'calendar/control/activity/{0}/{1}.html'
