...
def db_events(target, start_date, db_values, dkey):
    ...
    if event_name == 'event5_text':        
        ...
        for key in sorted(event_options):
            #print '<br/>', event_start, 'value:', event_value, '=?', key[3:] #match key excluding 01_, 02_
...
def confirm_page(f1, f2, title_msg):
    ...
    target.write('''<tr><td class="filler" colspan="3" align="center">{0}</td></tr><tr><td class="main" colspan="3" align="center">Reload the calendar month page(s) as needed to view your changes.</td></tr>\n<tr><td colspan="3" align="center">\n<form action="{1}" method="post"><input type="submit" name="Submit" value="Return"></td></tr></form>\n</table></body></html>\n'''.format(title_msg, litcon_this_month))        
    target.close()  
...
if __name__ == "__main__":
    ...
    if 'modify' in form:
            ...
            for dt in date_range:
                ...
                for dkey in sorted(sub_new):
                    ...
                    if dkey not in omit:
                        if len(key_values) == 5:
                            event_name = key_values[0]
                            #print '<br/>what now?', dkey, key_values, len(key_values)
                            ...
                            for id_rcd in event_data:
                                ...
                                if event_record == event_name:
                                    #print '<br/>what next?', dkey, key_values, len(key_values), event_name
        ...
        if len(headers) > 0:
            ...
            title_msg = ('''You may edit the liturgy schedule starting on {0} {1} ({2}). <br/> <br/>Please select / complete all liturgical activities that apply.'''.format(f1_weekday, fmt_start, hd_header)) 
            edit_day(start_date, title_msg, action='start')

        else:
            title_msg = ('''You may edit the liturgy schedule starting on {0} {1}.<br/><br/>Please select / complete all liturgical activities that apply.'''.format(f1_weekday, fmt_start)) 
            edit_day(start_date, title_msg, action='start')   
