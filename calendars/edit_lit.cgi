...
import lit_mgmt
import email_out
from config_cal import base_path, edit_lithead, lit_db, HD_db, email_from, email_to, mail_path
...
event_options = {'00_Select':'Select', '01_Mass':'Mass', '02_Mass_(Hispanic)':'Mass (Hispanic)', \
                 '03_Religious_Education':'Religious Education', '04_Eucharistic_Adoration':'Eucharistic Adoration', \
                 '05_Vigil_Mass':'Vigil Mass', '06_Confession':'Confession', \
                 '07_High_Mass':'High Mass (Holy Day)', '08_Stations':'Stations of the Cross'}   
...
def add_record(ilist):
    ...
    try:
        ...
        print('Could not add to the database! Please contact {0}'.format(email_from))
        sys.exit(1) 
            ...
            if sub in ['start_date', 'end_date']:
                ...
                if num == str(key):
                    if sub.endswith('name'): 
                        #print '<br/>?', sub #event2_name...why is this necessary? 
                        if int(num) == 5:
...
def compare_dict(event_data, dts):
    ...
    for id_rcd in event_data:
        ...
        #compare 2018-11-04 11:00 in db with dts 2018-11-04 
...
def db_events(target, start_date, db_values, dkey):
    ...
    #print '<br>why?', event_name, dkey #event1_menu, event2_menu #why do you need the key without a value? values are empty on load
    ...
    target.write('''<td class="main" align="center">
          ...
          <input type="submit" name="delete{2}" value="Delete"></td></tr>\n'''.format(dkey, dkey, dkey)) 
...
def sub_events(target, key_values, dkey):    
    ...
    target.write('<td class="main" align="center"></td></tr>\n')
...
def get_feedback(key_values):
    ...
    but to delete an event, key_values = [event_value, event_start] '''
...
def edit_day(start_date, html_range, title_msg, action='start'):
    ...
    '''event_data retrieves liturgical events for this day/dt from the form (if populated) or the db table
       ...
       set_add and event_data are global vars'''
    #edit_lithead is now a string in config file; 
    target = open(lit_edit, 'w')
    target.write(edit_lithead)
    target.write('''\n<tr><td class="main" colspan="2"><b>Start date</b>: {0} {1}</td>
    <td class="main" colspan="4"><b>End date</b>: {2} \n
    <select class="select_menu" name="end_date">\n'''.format(f1_weekday, fmt_start, f1_weekday))
    for dt in html_range:
        fmt_date = dt.strftime("%d %b %Y")
        dt_str = datetime.datetime.strftime(dt, '%Y-%m-%d')
        #print dt_str, sub_flds['start_date'], '<br/>'
    ...
    try:      
        ...
        print(t.substitute(start_date=start_date,month_path=litcon_this_month,title_msg=title_msg))
...
def confirm_page(full_list, f1, f2):  
    ...
    if not full_list:
        target.write('''<tr><td class="filler" colspan="3" align="center">There was an error, no events were read. <br/>
        Please try again, or report this error.''')
        
    else:
        target.write('<tr><td class="filler" colspan="3" align="center">You have changed the liturgy schedule to:')    
        for entry in full_list:  
            if isinstance(entry, basestring):
                if '_' in entry:
                    entry.replace('_', ' ') #fails
    ...
    for m in [f1, f2]:
        ...
        date_start = m.replace(day=1).date()
        #print year_num, month_name, date_start, f2, '<br/>'
...
if __name__ == "__main__":
    ...
    if 'modify' in form:
        ...
        if len(date_range) > 0:
            for dt in date_range:
                ...
                for dkey in sorted(sub_new):
                    key_values = get_keyValues(sub_new, dkey)
                    #print '<br/>what?', dkey, key_values #2 ['event2_menu', 'Mass_(Hispanic)', '2019-11-08 17:30', '', 'SHC'] ...why not used to update the db???
                    omit = get_omitValues(sub_new, dkey)
                    if dkey not in omit:
                        if len(key_values) == 5:
                            event_name = key_values[0]
                            #check for event_name key in event_data; event_name = event1_menu, etc == Input_Name
                            for id_rcd in event_data:
                                #db columns:[Event_Start, Input_Name, Liturgy_Event, Event_End, Event_Location]
                                ...
                                if event_record == event_name:
                                    ...
                                    if dto_db.date() == dt:
                                            ...
                                            statement = get_feedback(key_values)
                                            #full_list statement omits '_'; example: Mass (Hispanic) 2019-11-08 17:30 SHC 
        ...
        if len(all_errors) > 0:
            ...
            for mlist in mod_list:
                if len(mlist) == 6:
                    #print '<br/>?', mlist #now it's ok - db shows Hispanic Mass
                    modify_record(mlist) 
            
            for ilist in insert_list:
                if len(ilist) == 5:
                    #print '<br/>?', ilist #now it's ok - db shows Hispanic Mass
                    add_record(ilist)
    ...
    #deletion based on event ID: delete events one at a time;        
    elif 'delete1' in form: 
        end_date = sub_flds['end_date']
        f1 = datetime.datetime.strptime(start_date, '%Y-%m-%d')
        f2 = datetime.datetime.strptime(end_date, '%Y-%m-%d')
        fmt_end = f2.strftime("%-d %B %Y") 
        date_range = gen_range(f1, f2)
        full_list = delete_buttons(f1, f2, event='event1_menu')
        confirm_page(full_list, f1, f2)   
        ...
        fmt_end = f2.strftime("%-d %B %Y")        
        date_range = gen_range(f1, f2)
        full_list = delete_buttons(f1, f2, event='event2_menu')
        confirm_page(full_list, f1, f2)        
        ...
        fmt_end = f2.strftime("%-d %B %Y")        
        date_range = gen_range(f1, f2)
        full_list = delete_buttons(f1, f2, event='event3_menu')
        confirm_page(full_list, f1, f2)       
        ...
        fmt_end = f2.strftime("%-d %B %Y")        
        date_range = gen_range(f1, f2)
        full_list = delete_buttons(f1, f2, event='event4_menu')
        confirm_page(full_list, f1, f2)     
        ...
        if len(headers) > 0:
            ...
            title_msg = ('''You may edit the liturgy schedule starting on {0} {1} ({2}). <br/> <br/>
                            Please select / complete all liturgical activities that apply.'''.format(f1_weekday, fmt_start, hd_header)) 
            edit_day(start_date, html_range, title_msg, action='start')
            
        else:
            title_msg = ('''You may edit the liturgy schedule starting on {0} {1}. <br/> <br/>
                            Please select / complete all liturgical activities that apply.'''.format(f1_weekday, fmt_start)) 
            edit_day(start_date, html_range, title_msg, action='start')            
